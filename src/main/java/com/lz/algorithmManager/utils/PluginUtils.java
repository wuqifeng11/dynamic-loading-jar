...
public class PluginUtils {
    ...
    //线程安全的hashMap
    private static Map<String, ClassLoader> cache = new ConcurrentHashMap<String, ClassLoader>();

    //通过插件名称得到该插件的jar的URL
    private static URL listFileByPluginName(String algorithmName) throws MalformedURLException {
        ...
        String confDir = dir+"/"+"src/main/resources/"+"algorithmBase/" + "/" + algorithmName + ".jar";
        File jar = new File(confDir);
        if (!jar.exists()) {
            throw new RuntimeException("Plugin not found: " + algorithmName);
        }
